...
public class DetailDialog extends JFrame {
    ...
    public DetailDialog(CityGrid cityGrid, GridPanel gridPanel, int i, int j) {
        ...
        if (cityGrid.getPeople(i, j) == 0) {
            ...
            Image rawImage = null;
            if(gridPanel.getButton(i, j).getBackground() == Color.RED){
                rawImage = new ImageIcon("src\\pm25\\image\\angry.png").getImage();
            }
            if(gridPanel.getButton(i, j).getBackground() == Color.ORANGE){
                rawImage = new ImageIcon("src\\pm25\\image\\bad.png").getImage();
            }
            if(gridPanel.getButton(i, j).getBackground() == Color.YELLOW){
                rawImage = new ImageIcon("src\\pm25\\image\\better.png").getImage();
            }
            if(gridPanel.getButton(i, j).getBackground() == Color.GREEN){
                rawImage = new ImageIcon("src\\pm25\\image\\good.png").getImage();
            }
